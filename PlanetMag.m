...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for opt=opts
        for MPopt=MPopts
            ...
            if SPHOUT
                BeType = 'Be1sph_';
                varNames = {'exc name', 'period(hr)', 'B0r(nT)', 'B0th(nT)', 'B0phi(nT)', ...
                    'Ber_Re(nT)', 'Ber_Im(nT)', 'Beth_Re(nT)', 'Beth_Im(nT)', 'Bephi_Re(nT)', ...
                    'Bephi_Im(nT)'};
            else
                BeType = 'Be1xyz_';
                varNames = {'exc name', 'period(hr)', 'B0x(nT)', 'B0y(nT)', 'B0z(nT)', ...
                    'Bex_Re(nT)', 'Bex_Im(nT)', 'Bey_Re(nT)', 'Bey_Im(nT)', 'Bez_Re(nT)', ...
                    'Bez_Im(nT)'};
            end
            outFname = fullfile(outData, [BeType moonName '_' era '_' fEnd '.txt']);
            outTable = table(BD.fNames', T_h', B0vec1', B0vec2', B0vec3', real(B1vec1)', ...
                imag(B1vec1)', real(B1vec2)', imag(B1vec2)', real(B1vec3)', imag(B1vec3)', ...
                'VariableNames', varNames);
            writetable(outTable, outFname, 'delimiter', ',')
